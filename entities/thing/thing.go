...
/*
 Package thing implements the base type of all entities in WolfMUD. Each
 Thing is created with a unique ID. This aids identifying when two Things
 are the same Thing no matter which Interface the Thing is 'seen through'
 or which embed type is in use. Things can easily be compared in one of two
 ways:
...
/*
 ...
 otherwise false. The comparison is case insensitive. The passed alias to
 be checked will be trimmed of leading and trailing whitespace and
 uppercased before the comparison.

 This method is not optimised as we usually expect only 2 or 3 aliases. If
 there is the need for a HUGE number of aliases we may want to store hashes
 to save memory and/or use a map with the aliases as the keys and simply
...
/*
 IsAlso tests two things to see if one of them 'is also' the other - hence
 the functions name.

 WolfMUD uses a lot of Interfaces and embedded types. So we may be
 comparing, for example, a Player with a Mobile. However this causes
 issues:
