...
func Read(s *state) {
	...
	if len(s.words) == 0 {
		s.msg.Actor.Send("Did you want to read something specific?")
		return
	}
	...
	if what == nil {
		s.msg.Actor.Send("You see no '", name, "' to read.")
		return
	}
	...
	// Was writing found?
	if writing == "" {
		s.msg.Actor.Send("You see no writing on ", name, " to read.")
		return
	}
	...
	s.msg.Observer.Send("You see ", who, " read ", name, ".")
