...
func TestQuota_Query(t *testing.T) {
	...
	for off := 0; off < 256; off++ {
		ip := "127.0.0." + strconv.Itoa(off)
		verify(t, qq.Query, ip, 4, UnderQuota, Expired)
	}
	...
	for off := 0; off < 256; off++ {
		...
		// Push over quota
		qq.Quota(ip)
		verify(t, qq.Query, ip, 4, OverQuota, Valid)
	}
...
func TestQuota_CacheSweep(t *testing.T) {
	...
	qq.CacheSweep()
	verify(t, qq.Query, ip, -1, false, false)
}
...
func TestQuota_Quota_Window(t *testing.T) {
	...
	verify(t, qq.Query, ip, 4, UnderQuota, Valid)
...
func TestQuota_Quota_WithTimeout(t *testing.T) {
	...
	// Let expire
	time.Sleep(config.Quota.Timeout)
	verify(t, qq.Query, ip, 4, OverQuota, Expired)
